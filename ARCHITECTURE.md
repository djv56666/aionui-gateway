...
| Agent 容器 | **每 Agent 独立容器 + opencode serve** | 隔离彻底，一个 Agent 失控不影响其他；容器编排层天然处理启停、重启、资源限制 |
...
`[agent-runtime-opencode/sidecar/acp-bridge.js](agent-runtime-opencode/sidecar/acp-bridge.js)` - ACP Bridge 实现
...
```
opencode 重启时，ACP Bridge 向客户端发送的内部通知：

  { jsonrpc: '2.0', method: 'bridge/status',
    params: { status: 'restarting', reason: 'opencode exited with code 1' } }
  → 客户端应暂停发送请求，等待 ready
...
```
Agent 容器（每个 Agent 一个，统一镜像，环境变量区分）
┌──────────────────────────────────────────────────────┐
│                                                      │
│  Sidecar（轻量管理进程，前台运行）                      │
│  ├── 心跳上报（→ Gateway，每 30s）                     │
│  ├── cron 调度（通过内部通道触发 opencode，不走 HTTP）  │
│  ├── 用量采集（聚合后批量上报 Gateway）                  │
│  ├── 通知推送（任务完成后推送至企微/钉钉/邮件）          │
│  └── 健康检查 HTTP（:3000，Docker 健康探针用）          │
│                                                      │
│  Sidecar ACP Bridge（监听 :3001）                     │
...
│                                                      │
│  /skills/                  # Skill 只读挂载             │
│  /config/base.yaml         # 企业级 opencode 基础配置   │
└──────────────────────────────────────────────────────┘
```

### 容器镜像 & 启动流程

`[agent-runtime-opencode/Dockerfile](agent-runtime-opencode/Dockerfile)` - 容器镜像定义

- 基于 Node.js 20-slim
- 安装 opencode CLI
- 安装 Sidecar
- 暴露 3000（健康检查）和 3001（ACP Bridge）端口
- 非 root 用户运行

`[agent-runtime-opencode/entrypoint.sh](agent-runtime-opencode/entrypoint.sh)` - 容器启动脚本

- 启动 Sidecar（前台，作为 PID 1）
- Sidecar 内部启动 opencode acp 和 ACP Bridge

### Sidecar 实现

`[agent-runtime-opencode/sidecar/agent-sidecar.js](agent-runtime-opencode/sidecar/agent-sidecar.js)` - Sidecar 主逻辑
...
│   └── vite.config.ts
│
├── agent-runtime-opencode/            # Agent 容器镜像源码（独立镜像）
│   ├── sidecar/
│   │   ├── [agent-sidecar.js](agent-runtime-opencode/sidecar/agent-sidecar.js) - Sidecar 主逻辑（心跳/cron/用量/通知）
│   │   └── [acp-bridge.js](agent-runtime-opencode/sidecar/acp-bridge.js) - stdio ↔ WebSocket 桥接
│   ├── [index.js](agent-runtime-opencode/index.js) - Sidecar 入口
│   ├── [entrypoint.sh](agent-runtime-opencode/entrypoint.sh) - 容器启动脚本
│   ├── [Dockerfile](agent-runtime-opencode/Dockerfile) - 容器镜像定义
│   └── package.json
│
...
- Agent 容器镜像构建（opencode acp + Sidecar + ACP Bridge）
- Sidecar ACP Bridge（stdio ↔ WebSocket 桥接）
